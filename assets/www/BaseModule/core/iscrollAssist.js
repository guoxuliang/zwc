...
;define(function(require, exports, module){
	...
	var exportsMethods = {
		...
		newVerScrollForPull : function(wrapper,pulldownAction,pullupAction,opts,pullText){
			...
			var $pulldown = $wrapper.find('#aigov-list-pulldown'),
				$pullup   = $wrapper.find('#aigov-list-pullup'),
				pullupOffset   = 0,
				pulldownOffset = 0;
			
			if($pulldown.length>0){
				pulldownOffset = $pulldown.outerHeight();
				$pulldown.find('#aigov-list-pulldown-label').html(pulldownRefresh);
			}
			
			if($pullup.length>0){
				pullupOffset = $pullup.outerHeight();
				$pullup.find('#aigov-list-pullup-label').html(pullupLoadingMore);
			}
			...
			scrollObj.on('refresh',function(){
				...
				if ($pulldown.length>0 && $pulldown.hasClass('loading')) {
					...
					$pullup.find('#aigov-list-pullup-icon').show();
			...
			scrollObj.on('scrollMove',function(){
				...
				if ($pulldown.length>0 && this.y > 5 && !$pulldown.hasClass('flip')) {
					...
					$pulldown.find('#aigov-list-pulldown-label').html(releaseToRefresh);
					this.minScrollY = 0;
				...
				//this.y < this.minScrollY代表是上拉,以防下拉的时候未拉到尽头时进入上拉的逻辑中
					...
					$pullup.find('#aigov-list-pullup-label').html(releaseToLoading);
			...
			scrollObj.on('scrollEnd',function(){
				...
				if ($pulldown.length>0 && $pulldown.hasClass('flip')) {
					...
					$pulldown.find('#aigov-list-pulldown-label').html(loading);
					...
					$pullup.find('#aigov-list-pullup-label').html(loading);

...
public class Iso7816 {
	...
	public final static class Tag {
		...
		public Response verify() {
			final byte[] cmd = { //002000000 1234
					(byte) 0x00, // CLA Class
					(byte) 0x20, // INS Instruction
					(byte) 0x00, // P1 Parameter 1
					(byte) 0x00, // P2 Parameter 2
					(byte) 0x02, // Lc
					(byte) 0x12, (byte) 0x34, };//个人密码固定
			Log.e("--INFO--", "--verify 0020指令 input--"+Arrays.toString(cmd));
			Log.e("--INFO--", "--verify 0020指令 Hex:"+Util.byte2HexString(cmd));
		...
		public Response getMessage() {
			...
			Log.e("--INFO--", "--getMessage 80CA指令 input--"+Arrays.toString(cmd));
			Log.e("--INFO--", "--getMessage 80CA指令 Hex:"+Util.byte2HexString(cmd));
...
//					(byte) 0x44, (byte) 0x55, (byte) 0x66, (byte) 0x0F, // Le
		...
		public Response selectByName(byte... name) {
			ByteBuffer buff = ByteBuffer.allocate(name.length + 6);
			buff.put(
					(byte) 0x00) // CLA Class       READ
					.put((byte) 0xA4) // INS Instruction
					.put((byte) 0x04) // P1 Parameter 1
					.put((byte) 0x00) // P2 Parameter 2
					.put((byte) name.length) // Lc
					.put(name).put((byte) 0x00); // Le
			Log.e("--INFO--", "--公共钱包应用: 00A4指令 input--"+Arrays.toString(buff.array()));
			Log.e("--INFO--", "--公共钱包应用: 00A4指令 Hex:"+Util.byte2HexString(buff.array()));
			return new Response(transceive(buff.array()));
		}

		        //获取 伪随机数  MAC1
				public Response initializeForLoad(InitializeForLoadData data) {
					final byte[] cmd = {
							(byte) 0x80, // CLA Class
							...
							(byte) 0x0B,  // Lc
							(byte) 0x01,  //密钥索引号 1字节
							(byte) 0x00,(byte) 0x00,(byte) 0x00,(byte) 0x01,  //交易金额   4字节1  00000001
							(byte) 0x00,(byte) 0x50,(byte) 0xC2,(byte) 0x31,(byte) 0xB5,(byte) 0xC7,//终端机编号 6字节0050C231B5C7
							(byte) 0x00,     // Le
					};
					//cmd[cmd.length]=(byte)0x11;
					Log.e("--INFO--", "--Initialize For Load 8050指令 input--"+Arrays.toString(data.getALL()));
					Log.e("--INFO--", "--Initialize For Load 8050指令 Hex:"+Util.byte2HexString(data.getALL()));

					return new Response(transceive(data.getALL()));
				}
				public Response CREDITFORLOAD(CreditForLoadData data) {
					final byte[] cmd = { 
							(byte) 0x80, // CLA Class
							...
							(byte) 0x0b, // Lc
							(byte) 0x20,(byte) 0x16,(byte) 0x03, (byte) 0x25,   // 4字节  Data日期20160621
							(byte) 0x12,(byte) 0x27,(byte) 0x46,   // Data 时间  3 字节 12 27 46
							(byte) 0x24,(byte) 0x3D,(byte) 0x9C,(byte) 0xF4,// Data MAC2 243D9CF4504947FD
							
							(byte) 0x00,//Le
					};
					Log.e("--INFO--", "--CREDIT FOR LOAD 8052指令 input--"+Arrays.toString(data.getALL()));
					Log.e("--INFO--", "--CREDIT FOR LOAD 8052指令 Hex:"+Util.byte2HexString(data.getALL()));
					
					return new Response(transceive(data.getALL()));//8 
				}
				
		public void connSect() throws Exception{
			nfcTag.connect();
		}
		...
		public void close() {
			try {
				...
				e.printStackTrace();
				Log.e("INFO","--close eror--"+e.getMessage());
			}
		}

		public byte[] transceive(final byte[] cmd) {
			try {
				Log.e("--TR--", "--TR--"+Arrays.toString(cmd));
